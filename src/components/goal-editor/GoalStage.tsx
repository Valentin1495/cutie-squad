...
export const GoalStage = forwardRef<HTMLElement, GoalStageProps>(function GoalStage({
  addPhase,
  canSubmit,
  crewSlots,
  enteringCrewIndex,
  ...
  const occupiedCount = crewSlots.filter((slot) => slot.status !== "exiting").length;
  const isStageReady = canSubmit || isCompletionPreview;
  const boardHeight = occupiedCount === 0 ? 144 : occupiedCount < MIN_GOALS ? 158 : 174;
  ...
  const shouldShowStageBubble = !isGoalAddInProgress;
  const stableOccupiedCount =
    isGoalAddInProgress && enteringCrewIndex != null ? enteringCrewIndex : occupiedCount;
  const latestOccupiedCrewIndex = Math.max(occupiedCount - 1, 0);
  const stableStageReady =
    !isGoalAddInProgress && (isStageReady || occupiedCount >= MIN_GOALS || isCompletionPreview);
  const activeCrewIndex =
    isGoalAddInProgress
    ...
    isLaunching
      ...
      : stableStageReady
        ...
        : stableOccupiedCount === 0
          ? "waiting"
          : "settled";
  const stageBubbleMessage = getCrewStageLine(activeCrewIndex, stageDialoguePhase);
  const bubbleHeader = {
    message: stageBubbleMessage,
  };
  const [displayedBubbleHeader, setDisplayedBubbleHeader] = useState(bubbleHeader);
  const [isBubbleVisible, setIsBubbleVisible] = useState(!isGoalAddInProgress);
  ...
  useEffect(() => {
    ...
    if (wasGoalAddInProgressRef.current) {
      setIsBubbleVisible(false);
      const timeoutId = window.setTimeout(updateBubbleHeader, BUBBLE_UPDATE_DELAY);
    ...
    >
          ...
          STAGE_SPARKLES.slice(0, occupiedCount).map((sparkle, index) => (
            <motion.span
              ...
              style={{
                ...
                height: sparkle.size,
                borderRadius: 2,
                backgroundColor: index % 2 === 0 ? colors.yellow400 : colors.blue300,
      ...
      <motion.div
        ...
        style={{
          ...
          borderStyle: "solid",
          borderWidth: 1,
          background: "linear-gradient(180deg, #fff 0%, #f4f6ff 54%, #fff7ed 100%)",
          display: "flex",
          alignItems: "flex-end",
      ...
      >
        ...
        <motion.span
          animate={{
            opacity: occupiedCount >= MIN_GOALS || isCompletionPreview ? 0.42 : 0.18,
            scaleX: occupiedCount >= MIN_GOALS || isCompletionPreview ? 1.08 : 1,
          }}
          style={{
            position: "absolute",
            left: "12%",
            ...
            borderRadius: "50%",
            background:
              `radial-gradient(ellipse at center, ${withAlpha(colors.yellow300, 0.55)}, ${withAlpha(colors.yellow300, 0)} 68%)`,
            pointerEvents: "none",
          }}
        />
        ...
        {Array.from({ length: MAX_GOALS }, (_, index) => {
          ...
          const crewColor = CREW_COLORS[(slot?.crewIndex ?? index) % CREW_COLORS.length];
          const joined = slot != null && slot.status !== "exiting";
          const visibleCrew = slot != null;
          const isEnteringCrew = isCrewEntering && slot?.status === "entering";
          const isExitingCrew = slot?.status === "exiting";
          const dist = enteringCrewIndex == null ? 0 : Math.abs(index - enteringCrewIndex);
          const isDominoCrew =
            isCrewEntering &&
            enteringCrewIndex != null &&
            slot != null &&
            slot.status === "settled";
          const missionCrewIndex = activeMission?.targetCrewIndex ?? enteringCrewIndex;
          const isAiming = addPhase === "aim" && missionCrewIndex != null && index === missionCrewIndex;
          ...
          const isAimSupporter =
            addPhase === "aim" && enteringCrewIndex != null && joined && index !== enteringCrewIndex;
          ...
          const idleRotate =
            index % 3 === 0 ? [-2, 2, -2] : index % 3 === 1 ? [1, -2, 1] : [0, 0, 0];
              ...
              : crewMood === "ready"
                ...
                : crewMood === "celebrating"
                  ? {
                      ...
                      scale: moodScale,
                      opacity: 1,
                      rotate: canUseMoodLoop ? [0, index % 2 === 0 ? 8 : -8, 0] : 0,
                    }
                  : {
                      x: 0,
                      y: idleY,
          ...
          const aimSupporterDirection =
            enteringCrewIndex == null ? 0 : index < enteringCrewIndex ? 8 : -8;
              ...
              <motion.div
                ...
                style={{
                  ...
                  boxShadow: isSpeakingCrew
                    ...
                    : `inset 0 -6px 12px ${
                        joined ? withAlpha(crewColor, 0.15) : withAlpha(colors.grey900, 0.16)
                      }`,
                }}
              />
              <AnimatePresence>
                {isReceivingMission && (
                  <motion.div
                    ...
                    transition={{ delay: isAiming ? 0.08 : 0, duration: 0.28, ease: "easeOut" }}
                    style={{
                      position: "absolute",
                      bottom: 64,
                  ...
                  >
                    <div
                      style={{
                        ...
                        height: 18,
                        borderRadius: 999,
                        backgroundColor: withAlpha(activeMission.crewColor, 0.14),
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <span style={{ fontSize: 20, lineHeight: 1 }}>{activeMission.emoji}</span>
                      <span
                        style={{
                          minWidth: 0,
                          overflow: "hidden",
                    ...
                    <span
                      style={{
                        ...
                        height: 2,
                        borderRadius: 999,
                        backgroundColor: withAlpha(activeMission.crewColor, 0.72),
                ...
                <motion.span
                    ...
                    animate={
                      isLaunching
                        ...
                        : isExitingCrew
                          ? { x: -60, y: 0, scale: 0.7, opacity: 0, rotate: -12 }
                          : isEnteringCrew
                            ? shouldReduceMotion
                            ...
                            : isAiming
                              ? shouldReduceMotion
                                ? { x: 0, y: 0, scale: poseScale, opacity: 1, rotate: 0 }
                                : {
                                    x: 0,
                                    y: [0, -10, 4, 0],
                            ...
                            : isDominoCrew
                              ? shouldReduceMotion
                                ...
                                : {
                                    ...
                                    scale: [
                                      poseScale,
                                      poseScale + Math.max(0, 0.12 - dist * 0.03),
                                      poseScale,
                                    ],
                                    opacity: 1,
                                    rotate: [
                              ...
                              : isExcited
                                ? {
                                    x: 0,
                                    y: [0, hopHeight, 0],
                                    scale: [poseScale, poseScale + proximity * 0.04, poseScale],
                                    opacity: 1,
                              ...
                              : isCompletionPreview
                                  ? {
                                      x: 0,
                                      y: [0, -18, 0],
                                      scale: [poseScale, poseScale + 0.1, poseScale],
                                      opacity: 1,
                                ...
                                : isCompletionPreview
                                  ...
                                  : crewMood === "ready"
                                    ...
                                    : crewMood === "celebrating"
                                      ...
                                      : 1.8 + index * 0.18,
                      ease: "easeOut",
                      type: isEnteringCrew || isExcited || isAiming ? "spring" : undefined,
                      stiffness: isAiming ? 420 : isEnteringCrew ? 420 : isExcited ? 360 : undefined,
                      damping: isAiming ? 20 : isEnteringCrew ? 22 : isExcited ? 16 : undefined,
                      repeat:
                        canUseMoodLoop && (crewMood === "ready" || crewMood === "celebrating")
                          ? Infinity
                          : crewMood === "waiting" &&
                              !isCompletionPreview &&
                              !isLaunching &&
                  ...
                  style={{
                    ...
                    zIndex: isAiming ? 8 : 2,
                    willChange: isCrewAnimating ? "transform, opacity" : undefined,
                    }}
                >
                  {slot.emoji}
                </motion.span>
        ...
        <AnimatePresence>
          {isLaunching && (
            <motion.div
              key="launch-board"
              initial={{ opacity: 0, y: 4, scale: 0.72, rotate: -4 }}
              animate={{
                opacity: 1,
                y: [4, -42, -36],
                scale: [0.72, 1.08, 1],
              ...
              style={{
                ...
                borderRadius: 16,
                backgroundColor: colors.white,
                border: `2px solid ${withAlpha(colors.purple500, 0.28)}`,
                boxShadow: "0 14px 26px rgba(0,27,55,0.14)",
                padding: "13px 12px",
                textAlign: "center",
